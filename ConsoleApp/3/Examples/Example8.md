...
> All these classes are included in the [I/O Utitlity Classes](../../../IOUtils/1/API.md) download.

* [_TPJPipe_](../../../IOUtils/1/API/TPJPipe.md): to handle pipe i/o and to ensure pipes are opened with [inheritable](../InheritableHandles.md) handles.
* [_TPJFileHandle_](../../../IOUtils/1/API/TPJFileHandle.md): to open the input file with an [inheritable](../InheritableHandles.md) handle.
* [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md): to parse the output from the pipe into lines of text. This class text from a single byte ANSI character set to be output through the pipe.

First add the _PJConsoleApp_, [_PJPipe_](../../../IOUtils/1/API/PJPipe.md), [_PJFileHandle_](../../../IOUtils/1/API/PJFileHandle.md) and [_PJPipeFilters_](../../../IOUtils/1/API/PJPipeFilters.md) units to the uses statement.
...
* We open a ANSI text file for input with an [inheritable](../InheritableHandles.md) handle using [_TPJFileHandle_](../../../IOUtils/1/API/TPJFileHandle.md). Replace `InputFile.txt` with some suitably long ANSI text file.
* Two default sized pipes are created with [inheritable](../InheritableHandles.md) handles using [_TPJPipe_](../../../IOUtils/1/API/TPJPipe.md).
* Two [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) objects are created to parse the output from the standard output and standard error pipes. Each object is passed the relevant pipe in its constructor and has an event handler set that is called after every complete line of text is read.
...
* Finally we free all the objects. Freeing the [_TPJPipe_](../../../IOUtils/1/API/TPJPipe.md) and [_TPJFileHandle_](../../../IOUtils/1/API/TPJFileHandle.md) objects closes the pipe and file handles respectively.
...
_WorkHandler_ calls the [_ReadPipe_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-ReadPipe.md) methods of the two [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) objects. This method simpy copies all available data from the associated pipe and processes it. Importantly, _WorkHandler_ also calls _Application.ProcessMessages_ to enable the memo controls to update.

_CompletionHandler_ causes the [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) objects to flush any remaining text from their internal buffers, ensuring it gets written to the memo controls.

So far we haven't seen how the memo controls actually get updated. This is done in the final piece of code, and that's the implementation of the [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) objects' [_OnLineEnd_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-OnLineEnd.md) event handlers:
...
These events are triggered once for each complete line of text that the [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) read from the pipes. The events are also triggered for any remaining text when the application ends (in the calls to the [_Flush_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-Flush.md) method). Each event is handled simply by adding the line of text to the appropriate memo control.
...
The pipe objects are created on the fly in the [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) [constructor](../../../IOUtils/1/API/TPJPipeFilter-Create.md) calls. The optional second `True` parameter tells the filter object to free the pipe when it is destroyed. We use the [_Pipe_](../../../IOUtils/1/API/TPJPipeFilter-Pipe.md) property of the filter to access the pipe's [_WriteHandle_](../../../IOUtils/1/API/TPJPipe-WriteHandle.md) when setting the console application object's [_StdOut_](../API/TPJCustomConsoleApp-StdOut.md) and [_StdIn_](../API/TPJCustomConsoleApp-StdIn.md) properties.

The is one further bit of slimming down we can do, and that is to remove the console application object's [_OnComplete_](../API/TPJCustomConsoleApp-OnComplete.md) event handler. We can do this because [_TPJAnsiSBCSPipeFilter_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter.md) instance automatically calls the [_Flush_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-Flush.md) method when the object is being destroyed. This causes the [_OnLineEnd_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-OnLineEnd.md) event handler to be called with any remaining text. Therefore the explicit call to [_Flush_](../../../IOUtils/1/API/TPJAnsiSBCSPipeFilter-Flush.md) in the [_OnComplete_](../API/TPJCustomConsoleApp-OnComplete.md) handler is redundant.

...
Attempts to find the first element of an array for which a given constraint function returns `True`. Returns a _Boolean_
...
* _AItem_ - Set to the first element of _A_ for which _AConstraint_ returns `True`. Undefined if _AConstraint_ returns `False`, i.e. no such element is found.
...
The second example finds the first local maximum in an array of integers. To do this we need to be able to access the array element before and after a given element. Therefore the _TConstraintEx\<T\>_ overload of _TryFind\<T\>_ is required.
